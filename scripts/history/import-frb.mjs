...
if (!season) {
  console.error('Usage: node scripts/history/import-frb.mjs <season>')
  process.exit(1)
}

const sourceDir = path.resolve(`data-import/frb/${season}`)
...
function resolveManager(sourceName) {
...
const seasonTeams = teamsRows
  ...
  .map((row) => ({
    managerId: resolveManager(row['Owner Name']),
